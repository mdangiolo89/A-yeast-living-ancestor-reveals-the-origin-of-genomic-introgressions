...
## usage: perl genotypeassignerSpar.pl <coverage file> <vcf file> <markers list>
...
$possiblesnplist{$chrcorr}{$possnpcorr}{'DBV'}=$base1;
$possiblesnplist{$chrcorr}{$possnpcorr}{'CBS'}=$base2;
}
...
foreach my $possiblesnpposition (sort {$a <=> $b} keys %{$possiblesnplist{$chrname}}) {
		if ($covlist{$nameofchr}{$possiblesnpposition}>=50) {
			...
			if ($snplist{$possiblesnpposition}{'freq'}==1) {print $out "$possiblesnpposition,C\n"}
			elsif ($snplist{$possiblesnpposition}{'freq'}==0) {print $out "$possiblesnpposition,D\n"}
			else {print $out "$possiblesnpposition,H\n"}
		}
                else {print $out "$possiblesnpposition,D\n"}
		}
