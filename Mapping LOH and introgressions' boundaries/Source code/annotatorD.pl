...
## usage: perl annotatorD.pl <genotype file>
...
## start counting how many consecutive markers carry the same genotype: note that the type of markers counted can be changed commenting/discommenting the following 3 lines. Example: if countletters("D") is discommented the script will count how many consecutive markers have the D genotype (homozygous DBVPG6765)
countletters("D");
#countletters("C");
#countletters("H");
...
foreach my $keysregions (sort {$a <=> $b} keys %regions){
	...
	print $out "$chromosomename\t$regions{$keysregions}{'init'}\t$regions{$keysregions}{'end'}\t$regions{$keysregions}{'counter'}\t$letter\n";
	}                                                    
  }
}
